...
export const EagleAnimatedEmblem: React.FC = () => {
  // Levitación vertical continua suave
  const floatAnim = useSharedValue(0);
  // Inclinación cinética 3D
  ...
  const animatedEmblemStyle = useAnimatedStyle(() => {
    ...
    const rotateZ = `${interpolate(tiltAnim.value, [-1, 1], [-1.6, 1.6])}deg`;
  ...
  return (
    <View style={styles.container}>
      {/* Resplandor ambiental de luz holográfica difusa */}
      <Animated.View style={[styles.ambientGlow, animatedGlowStyle]}>
        <Svg width={290} height={290} viewBox="0 0 290 290">
          ...
          <Circle cx="145" cy="145" r="145" fill="url(#auraGlow)" />
        </Svg>
      </Animated.View>

      {/* Partículas de destellos brillantes orbitales */}
      <Animated.View style={[styles.sparkleParticle, animatedParticleStyle1]}>
        <SparkleIcon size={14} color="#00E5FF" />
      </Animated.View>
      ...
      <Animated.View style={[styles.sparkleParticle, animatedParticleStyle3]}>
        <SparkleIcon size={16} color="#38BDF8" />
      </Animated.View>

      {/* Águila 3D Cromada Holográfica con patas y garras completas visibles */}
      <Animated.View style={[styles.emblemWrapper, animatedEmblemStyle]}>
        <Image
          source={require("../../../../assets/images/eagle_3d_chrome.png")}
